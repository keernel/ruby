...
describe PagSeguro::PaymentRequest::RequestSerializer do
  context '#to_xml_params' do
    ...
    it 'should serializer primary receivers' do
      ...
      expect(subject.to_xml_params).to match /<primaryReceiver>.*<email>a@example.com<\/email>.*<\/primaryReceiver>/m
    end
    ...
    it 'should serializer other receivers' do
      ...
      expect(subject.to_xml_params).to match /<receivers>.*<receiver>.*<email>a@example.com<\/email>/m
    end
    ...
    it 'should serializer sender' do
      ...
      expect(subject.to_xml_params).to match /<sender>.*<name>Sender<\/name>.*<email>b@example.com<\/email>.*<phone>.*<areaCode>51<\/areaCode>.*<number>2<\/number>/m
    end
